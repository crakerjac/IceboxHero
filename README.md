...
- ~~**Derived state in IPC payload**~~ — **Done.** `sensor_service.py` now computes debounced per-sensor `state` (NORMAL/WARNING/CRITICAL) once, at read time, via the shared `derive_sensor_state()` in `config_helper.py`, and writes it directly into the IPC payload as `{"temp_f": ..., "state": ...}`. `display_service.py` and `alert_service.py` no longer track thresholds, holdoff counts, or read state independently — they just read `state` off the payload, so they can never drift apart. `mock_sensors.py` runs the same shared function so dev testing stays representative. Email cadence for sustained CRITICAL conditions now also matches WARNING (fires once per episode rather than repeating on cooldown) — see `alert_service.py`'s `sensor_critical_state`/`sensor_warning_state` flags.
